...
func TestIntegrationProcessTransaction(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			`{"title":"Validação da Adquirente ao Processar Transação","detail":"Valor inválido"}`,
		},
		{
			"Cielo - Transação Inválida - Valor inválido > 500",
			newAcquirerID_IT("Cielo"),
			newExternalTransactionDTO_IT("xpto121a", "João", 501, 1),
			http.StatusBadRequest,
			`{"title":"Validação da Adquirente ao Processar Transação","detail":"Valor inválido"}`,
		},
		{
			"Stone - Transação Inválida - Portador = João Antônio",
			newAcquirerID_IT("Stone"),
			newExternalTransactionDTO_IT("xpto121a", "João Antônio", 1000, 1),
			http.StatusBadRequest,
			`{"title":"Validação da Adquirente ao Processar Transação","detail":"Transação não permitida para o portador"}`,
		},
		{
			"Cielo - Transação Inválida - Portador = João Antônio",
			newAcquirerID_IT("Cielo"),
			newExternalTransactionDTO_IT("xpto121a", "João Antônio", 500, 1),
			http.StatusBadRequest,
			`{"title":"Validação da Adquirente ao Processar Transação","detail":"Transação não permitida para o portador"}`,
		},
		{
			...
			http.StatusBadRequest,
			`{"title":"Validação da Adquirente ao Processar Transação","detail":"Não aprovado"}`,
		},
		{
